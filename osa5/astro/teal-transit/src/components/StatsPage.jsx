          ...
          <ListGroup className='m-0 list-group-flush'>
            <ListGroup.Item>
              <motion.div animate={controls} custom={0} initial={{ opacity: 0, x: -50 }}>
                Most popular post: <small className='text-muted'>{ `${sortedByLikes[0].title} (${sortedByLikes[0].likes} likes)` }</small>
              </motion.div>
            </ListGroup.Item>
            <ListGroup.Item>
              <motion.div animate={controls} custom={1} initial={{ opacity: 0, x: -50 }}>
                Newest post: <small className='text-muted'>{ `${sortedByDate[0].title} (Added ${moment(sortedByDate[0].added).fromNow()}.)` }</small>
              </motion.div>
            </ListGroup.Item>
            <ListGroup.Item>
              <motion.div animate={controls} custom={2} initial={{ opacity: 0, x: -50 }}>
                Oldest post: <small className='text-muted'>{ `${sortedByDate[sortedByDate.length - 1].title} (Added ${moment(sortedByDate[sortedByDate.length - 1].added).fromNow()}.)` }</small>
              </motion.div>
            </ListGroup.Item>
          </ListGroup>
        </Card>
              ...
              <ListGroup className='list-group-flush border-top'>
                <ListGroup.Item>
                  <motion.div animate={controls} custom={3} initial={{ opacity: 0, x: -50 }}>
                    Most popular post: <small className='text-muted'>{ userBlogs.sort((a, b) => (b.likes - a.likes))[0].title }</small>
                  </motion.div>
                </ListGroup.Item>
                <ListGroup.Item>
                  <motion.div animate={controls} custom={4} initial={{ opacity: 0, x: -50 }}>
                    Posts created: <small className='text-muted'>{ userBlogs.length }</small>
                  </motion.div>
                </ListGroup.Item>
                <ListGroup.Item>
                  <motion.div animate={controls} custom={5} initial={{ opacity: 0, x: -50 }}>
                  Total post likes: <small className='text-muted'>{ userBlogs.reduce((acc, blog) => acc + blog.likes, 0) }</small>
                  </motion.div>
                </ListGroup.Item>
              </ListGroup>
            ) || <Card.Text>No posts created yet.</Card.Text>}
          </Card.Body>
